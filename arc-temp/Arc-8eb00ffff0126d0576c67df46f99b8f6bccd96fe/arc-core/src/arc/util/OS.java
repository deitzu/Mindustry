...
public class OS{
    ...
    static{
        // Check for Android via system property (for JVM-on-Android like Mojo/MJ Launcher)
        // or via standard Android Runtime detection
        boolean forceAndroid = "true".equalsIgnoreCase(propNoNull("arc.android")) || "1".equals(propNoNull("arc.android"));
        if(forceAndroid || propNoNull("java.runtime.name").contains("Android Runtime") || propNoNull("java.vm.vendor").contains("The Android Project") || propNoNull("java.vendor").contains("The Android Project")){
            isAndroid = true;
            isWindows = false;
            isLinux = false;
            isMac = false;
            // Don't force is64Bit = false on JVM-on-Android; let architecture detection work
        }
